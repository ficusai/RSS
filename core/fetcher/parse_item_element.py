...
def parse_item_element(elem: ET.Element, feed_config: Dict[str, Any], extract_full_text: bool = False) -> Dict[str, Any]:
    ...
    if extract_full_text and link and len(full_text_clean or preview_clean) < 150:
        try:
            from features.feature_web_scraper_fallback.implementation.web_scraper_fallback import fetch_full_page_text
            extracted = fetch_full_page_text(link, timeout=5)
            if extracted and len(extracted) > len(full_text_clean or ""):
                full_text_clean = extracted
        except Exception:
            pass
